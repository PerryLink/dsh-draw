...
- `src/event-gate.ts` — the adaptive session-event gate: append `draw/generated` only when the host knows the type or honors the `ignorable` envelope (probed at mount on a detached SessionStore, never host persistence); rc.6/rc.7 hosts and the envelope-less `0.1.2-alpha.1` host (which fails closed on unknown types at read) fail the probe and commits degrade to the fallback ledger.
