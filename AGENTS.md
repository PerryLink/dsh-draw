...
- `src/quota.ts` — per-session quota folded from the durable session log plus the in-memory fallback ledger (the log is the durable source of truth; the ledger keeps the live session exact on hosts that cannot log the event).
- `src/session-events.ts` — `draw/generated` `SessionEventMap` member (declaration merging into `@deepseek-ai/dsh-session/types`; the parent-module augmentation does NOT reach the interface) + append/fold helpers, the gated `commitDrawGenerated`, and the WeakMap-keyed in-memory fallback ledger.
- `src/event-gate.ts` — the adaptive session-event gate: append `draw/generated` only when the host knows the type or honors the `ignorable` envelope (probed at mount on a detached SessionStore, never host persistence); rc.6/rc.7 hosts and the `0.1.2` line (whose `Session.append` third argument is a `SurfaceIntent` for surface events and cannot stamp the `ignorable` marker, failing closed on unknown types at read) fail the probe and commits degrade to the fallback ledger.
...
- `tests/` — vitest; REAL `Context` + `SessionStore`/`Session`/`ToolRuntime` from the `0.1.7-alpha.2` peers, scripted transport (injected through the `dsh-draw/transport` seam), a real `AttachmentStore` subclass keeping images in memory, and a fake credential provider.
...
- **Quota is durable and log-folded.** Both axes fold the `draw/generated` events; the generation axis is checked before engine spend and the byte axis before attachment storage. **The log append is gated** (src/event-gate.ts): an unknown, unmarked event type would make the session refuse to reopen under the rc.6/rc.7 static whitelist (and the `0.1.2-rc.1` line fails closed on unknown types at read), so gated-off commits land in the in-memory fallback ledger — quota stays exact for the live session and resets on restart there. Never bypass the gate with a raw append in plugin paths.
